...
public:
    ...
    void chargerListeTram(Ligne* ligne); //Hugo
    /** Méthode qui permet de rajouter un tramway à l'indice idx */
    void ajouterTram(int idx, const Tramway& Tram); //Hugo
    /** Méthode qui supprime le tram à l'indice idx */
    void supprimerTram(int idx); //Gana
    /** Méthode qui trouve un tram */
    TramCS* trouveTram(const Tramway& tram)const; //Gana
    /** Méthode qui permet de tariter toute les trams de la liste */
    void TraitementListeTram(); //Ensemble
private:
    TramCS *d_tete;
};
